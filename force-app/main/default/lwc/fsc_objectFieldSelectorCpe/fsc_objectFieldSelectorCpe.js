...
export default class Fsc_objectFieldSelectorCpe extends LightningElement {
    ...
    connectedCallback() {
        ...
        Object.values(this.inputValues).forEach(inputVal => {
            inputVal = {
                ...inputVal,
                get buttonVariant() {
                    return this.useMergeField ? 'brand' : 'neutral'
                }
            }
        })
    }
        ...
        if (this._values?.length) {
            this._values.forEach(curInputParam => {
                if (curInputParam.name && this.inputValues[curInputParam.name]) {
                    ...
                    if (this.isMergeField(this.inputValues[curInputParam.name].value)) {
                        this.inputValues[curInputParam.name].useMergeField = true;
                    }
                }
            });
        }
    ...
    handleValueChange(event) {
        if (event.target) {
            ...
            if (event.detail.referenceValue) {
                curAttributeValue = event.detail.referenceValue;
            }
    ...
    handleChangeUseMergeField(event) {
        console.log('in handleChangeUseMergeField');
        let valueName = event.target.name;
        console.log('valueName = '+ valueName);
        this.inputValues[valueName].useMergeField = !this.inputValues[valueName].useMergeField;
        console.log(JSON.stringify(this.inputValues[valueName]));
    }
    ...
    isMergeField(value) {
        return value && value.substring(0,2) === '{!' && value.charAt(value.length - 1) === '}';
